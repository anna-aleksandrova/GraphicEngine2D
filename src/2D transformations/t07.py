...
if __name__ == '__main__':
    scene = AnimatedSceneSample(
        ...
        title="Task 7",  # заголовок рисунка
    ...
    translation1 = TranslationAnimation(  # створюємо анімацію переміщення
        end=vertex(-2.2, -2.2),  # значення точки у яку треба перемітити
    ...
    translation2 = TranslationAnimation(  # створюємо анімацію переміщення
        end=vertex(2.2, 2.2),  # значення точки у яку треба перемітити
        channel=FIGURE_KEY,  # ідентифікатор фігури до якої має застосовуватися анімація
        frames=20,  # кількість кадрів анімації
        )
    ...
    scene.add_animation(translation1)
